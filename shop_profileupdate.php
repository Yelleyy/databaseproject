...
<?php 
    ...
    $sql = "UPDATE `shop` SET Name_Shop='$Name_Shop',Pass_Shop='$Pass_Shop',Email_Shop='$Email_Shop',Tel_Shop='$Tel_Shop' WHERE ID_Shop=$ID_Shop"; 
    $result1 = mysqli_query($con, $sql) or die("Error in query: $sql " . mysqli_error($con));
    if ($result1) {
        echo "<script type='text/javascript'>";
        echo  "alert('แก้ไขเรียบร้อยเรียบร้อย');";
        echo "window.location='shop_profile.php';";
        echo "</script>";
    } else {
        echo "<script type='text/javascript'>";
        echo "window.location='shop_profile.php';";
